 ...
 */
class Sigma_Vc_Footer_Menu_Shortcode_Fields {
	/**
	 * Shortcode title.
	 *
	 * @since 1.0.0
	...
	function __construct() {
		$this->title         = esc_html__( 'Footer Menu', 'sigma-core' );
		$this->handle        = 'sigma_footer_menu';
		$this->description   = esc_html__( 'Use this shortcode to show a Footer Menu.', 'sigma-core' );
	...
	/**
	 * Footer Menu Bar shortcode mapping.
	 *
	 * @return void
	 */
	function shortcode_fields() {
      ...
      array(
        ...
        'edit_field_class' => 'vc_col-sm-12 vc_column',
			...
			array(
				'type' => 'dropdown',
				'heading' => esc_html__( 'Menu Orientation', 'sigma-core' ),
				'value' => array(
					esc_html__( 'Horizontal', 'sigma-core' ) => 'horizontal',
					esc_html__( 'Vertical', 'sigma-core' ) => 'vertical',
				),
				'param_name'  => 'footer_menu_orientation',
				'description' => esc_html__( 'Select the orientation of the menu', 'sigma-core' ),
			),
			array(
				'type' => 'dropdown',
				'heading' => esc_html__( 'Menu Alignment', 'sigma-core' ),
				'value' => array(
					esc_html__( 'Left', 'sigma-core' ) => 'aligned-left',
					esc_html__( 'Right', 'sigma-core' ) => 'aligned-right',
					esc_html__( 'Center', 'sigma-core' ) => 'aligned-center',
				),
				'param_name'  => 'footer_menu_alignment',
				'description' => esc_html__( 'Select the alignment of the menu', 'sigma-core' ),
				'dependency' => array(
					'element' => 'footer_menu_orientation',
					'value'   => 'horizontal',
				),
			),
			array(
				'type'       => 'param_group',
				'param_name' => 'footer_menu_items',
				'params'     => array(
					array(
						'type'             => 'textfield',
						'heading'          => esc_html__( 'Item Title', 'sigma-core' ),
						'param_name'       => 'footer_menu_title',
						'edit_field_class' => 'vc_col-sm-12 vc_column',
					),
					array(
						'type'        => 'vc_link',
						'heading'     => esc_html__( 'Item Link', 'sigma-core' ),
						'param_name'  => 'footer_menu_link',
					),
          array(
						'type'             => 'textfield',
						'heading'          => esc_html__( 'Item Badge', 'sigma-core' ),
						'param_name'       => 'footer_menu_badge',
						'edit_field_class' => 'vc_col-sm-12 vc_column',
					),
          array(
    				'type' => 'dropdown',
              ...
              esc_html__( 'Success', 'sigma-core' )       => 'success',
    				...
    				'param_name'  => 'footer_menu_badge_state',
	...
	/**
	 * Footer Menu shortcode callback functions.
	 */
	function shortcode_callback( $atts, $content = null, $handle = '' ) {
		$default_atts = array(
      'menu_title'  =>  '',
			'footer_menu_orientation'	=>	'horizontal',
			'footer_menu_alignment'	=>	'aligned-left',
			'footer_menu_items'          => '',
      'footer_menu_title' =>  '',
      'footer_menu_link'  =>  '',
      'footer_menu_badge' =>  '',
      'footer_menu_badge_state' =>  'success',
      ...
      'icon_flaticon'       => '',
		...
		$sigma_shortcodes_class_unique       = 'sigma-footer-menu-' . mt_rand();
		$sigma_shortcodes_classes            = $this->handle . '_wrapper';
		$sigma_shortcodes_classes            .= ' ' . $sigma_shortcodes_class_unique;
		$sigma_shortcodes_classes            .= ' footer-menu-' . $atts['footer_menu_orientation'];
		$sigma_shortcodes_classes            .= ' footer-menu-' . $atts['footer_menu_alignment'];
		...
		if ( $atts[ 'link_color' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . " ul.menu li a { color: " . $atts[ 'link_color' ] . ";}";
		}
		if ( $atts[ 'link_color_hover' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . " ul.menu li a:hover { color: " . $atts[ 'link_color_hover' ] . ";}";
		}
		if ( $atts[ 'title_color' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . ".sigma_footer_menu_wrapper .widgettitle { color: " . $atts[ 'title_color' ] . ";}";
		}
    ...
    if ( ! $atts['footer_menu_items'] ) {
    	return;
    }
    $list_items = vc_param_group_parse_atts( $atts['footer_menu_items'] );
		?>
      ...
      <ul class="menu">
        ...
        foreach ( $list_items as $list_item ) {
					...
					$icon = '';
          $title = isset( $list_item['footer_menu_title'] ) && ! empty( $list_item['footer_menu_title'] ) ? $list_item['footer_menu_title'] : '';
          $badge = isset( $list_item['footer_menu_badge'] ) && ! empty( $list_item['footer_menu_badge'] ) ? $list_item['footer_menu_badge'] : '';
          $badge_state = isset( $list_item['footer_menu_badge_state'] ) && ! empty( $list_item['footer_menu_badge_state'] ) ? $list_item['footer_menu_badge_state'] : 'primary';
          $list_link = isset($list_item['footer_menu_link']) && !empty($list_item['footer_menu_link']) ? vc_build_link( $list_item['footer_menu_link'] ) : '';
          if($list_link){
...
new Sigma_Vc_Footer_Menu_Shortcode_Fields();
