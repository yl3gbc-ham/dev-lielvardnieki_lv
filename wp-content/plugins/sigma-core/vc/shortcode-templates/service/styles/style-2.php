...
$service_image_data = wp_get_attachment_image_src( $atts[ 'content_bg_image' ], 'full' );
$service_image_url  = ( isset( $service_image_data[0] ) && $service_image_data[0] ) ? $service_image_data[0] : '';
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('sigma-service sigma-service-style-2 sigma-service-wrapper'); ?>>
	<div class="sigma_service style-1">
						...
						<?php } ?>
						<div class="service-style-2-content-wrapper" <?php if($service_image_url) { ?> style="background-image: url(<?php echo esc_url($service_image_url); ?>);"<?php } ?>>
	        	<?php if ( $atts[ 'show_icon' ] == 'yes' && isset($kb_service_icon) && !empty($kb_service_icon)) { ?>
