 ...
 * Event shortcode.
 */
class Sigma_Vc_Event_Details_Shortcode_Fields {
	/**
	 * Shortcode title.
	 *
	 * @since 1.0.0
	...
	function __construct() {
		$this->title         = esc_html__( 'Event Details', 'sigma-core' );
		$this->handle        = 'sigma_event_details';
		$this->description   = esc_html__( 'Use this shortcode to show event details.', 'sigma-core' );
	...
	function shortcode_callback( $atts, $content = null, $handle = '' ) {
		...
		//Color
		if ( $atts[ 'title_color' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . " .sigma-portfolio-thumbnail .sigma_portfolio-details .sigma_portfolio-details-item h5 { color: " . $atts[ 'title_color' ] . ";}";
		}
		if ( $atts[ 'subtitle_color' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . " .sigma-portfolio-thumbnail .sigma_portfolio-details .sigma_portfolio-details-item span { color: " . $atts[ 'subtitle_color' ] . ";}";
		}
		if ( $atts[ 'bg_color' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . " .sigma-portfolio-thumbnail .sigma_portfolio-details { background-color: " . $atts[ 'bg_color' ] . ";}";
		}
		if ( $atts[ 'border_color' ] ) {
			$inline_css .= "." . $sigma_shortcodes_class_unique . " .sigma-portfolio-thumbnail .sigma_portfolio-details .sigma_portfolio-details-item + .sigma_portfolio-details-item { border-color: " . $atts[ 'border_color' ] . ";}";
		}

		$sigma_shortcodes_classes .= ' event-details-' . $atts['style'];
		...
		<div <?php echo ( $atts['el_id'] ) ? 'id=' . esc_attr( $atts["el_id"] ) : ''; ?> class="<?php echo esc_attr( $sigma_shortcodes_classes ); ?>">
			<?php sigmacore_get_vc_shortcode_template( 'event-details/content' ); ?>
		</div>
		<?php
		return ob_get_clean();
	}
}
new Sigma_Vc_Event_Details_Shortcode_Fields();
