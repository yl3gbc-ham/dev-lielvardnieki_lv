<?php
/**
 * Widget API: Sigma_Widget_Recent_Services class
 *
 * @package WordPress
 * @subpackage Widgets
 * @since 4.4.0
 ...
 * @see WP_Widget
 */
class Sigma_Widget_Recent_Services extends WP_Widget {
	/**
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'                 => __( 'Your site&#8217;s most recent services.', 'sigma-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'recent-services', __( 'Sigma Recent Services', 'sigma-core' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		?>
		<ul>
			<?php foreach ( $r->posts as $recent_services ) : ?>
				<?php
				$post_title   = get_the_title( $recent_services->ID );
				$title        = ( ! empty( $post_title ) ) ? $post_title : __( '(no title)', 'sigma-core' );
				$aria_current = '';
				if ( get_queried_object_id() === $recent_services->ID ) {
					$aria_current = ' aria-current="page"';
				}
				?>
				<li>
					<?php if ( $show_image && has_post_thumbnail( $recent_services->ID ) ) : ?>
						<div class="sigma-services-image">
							<?php echo get_the_post_thumbnail( $recent_services->ID, 'thumbnail' ); ?>
						</div>
					<?php endif; ?>
					<div class="sigma-services-content">
						<a href="<?php the_permalink( $recent_services->ID ); ?>"<?php echo $aria_current; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php echo esc_html( $title ); ?></a>
						<?php if ( $show_date ) : ?>
							<span class="post-date"><?php echo get_the_date( '', $recent_services->ID ); ?></span>
						<?php endif; ?>
					</div>
				</li>
			<?php endforeach; ?>
	...
	public function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of services to show:', 'sigma-core' ); ?></label>
