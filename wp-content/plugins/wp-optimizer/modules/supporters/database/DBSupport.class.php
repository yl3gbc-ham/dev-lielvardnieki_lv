    ...
    public static function count($name, $args = array())
    {
        ...
        switch ($name) {
            ...
            case 'orphan_term_relationships':
                ...
                $count = $wpdb->get_var("SELECT COUNT(object_id) FROM $wpdb->term_relationships AS tr INNER JOIN $wpdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id WHERE tt.taxonomy NOT IN ('$excluded_taxonomies_sql') AND tr.object_id NOT IN (SELECT ID FROM $wpdb->posts)"); // WPCS: unprepared SQL ok.
                break;
            case 'unused_terms':
